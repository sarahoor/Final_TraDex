...
import { fetchPancakeV3Snapshot } from '@/lib/markets/pancakeswapV3';
...
export async function GET(req: Request) {
  try {
    ...
    const key = `pancake:${secondsAgo}:${first}`;
    if (cache && cache.key === key && Date.now() - cache.at < 60_000) {
      return NextResponse.json(cache.data, { status: 200 });
    }
    const data = await fetchPancakeV3Snapshot(secondsAgo, first);
    cache = { key, at: Date.now(), data };
    return NextResponse.json(data, { status: 200 });
  } catch (e: any) {
    console.error('PANCAKE route error:', e);
